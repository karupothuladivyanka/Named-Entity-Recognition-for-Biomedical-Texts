...
"""
    try:
        ...
        st.session_state['debug_filtered_entities'] = filtered_text  # Save for debugging
        # Parse the filtered response into entities
        filtered_entities = parse_gemini_response(filtered_text)
        return filtered_entities
    except Exception as e:
        st.warning(f"Error filtering entities for visualization: {str(e)}")
        return []

def visualize_entities(text, entities):
    """Display text with highlighted entities after filtering"""
    if not text or not entities:
    ...
    # For debugging, show how many entities are being visualized
    st.session_state['debug_entities_count'] = len(filtered_entities)
